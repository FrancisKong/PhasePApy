...
class FBSummary():
  ...
  def threshold(self):
    ...
    """
    dt = self.stats.delta
    npts_Tma = int(np.round(self.picker.t_ma / dt,0))
    LEN = self.stats.npts
  ...
  def pick_ident(self):
    ...
    # clean close picks
    window_t_up = int(np.round(self.picker.t_up/dt,0))
    ...
    for i in range(len(trigger_ptnl)):
      ...
      M = min(r, R)
      if N * np.std(self.tr.data[int(np.round(trigger_ptnl[i] / dt,0)) - M:int(np.round(trigger_ptnl[i] / dt,0))]) >= np.std(self.tr[int(np.round(trigger_ptnl[i] / dt,0)):int(np.round(trigger_ptnl[i] / dt,0)) + M]):
    ...
    for i in range(len(trigger)):
      index = int(np.round(trigger[i] / dt,0))
      while True:
        if self.summary[index] > self.summary[index - 1]:
          index -= 1
        else:
          break
      picks.append(UTCDateTime(self.tr.stats.starttime + np.round(t[index], 3)))
    ...
    for i in range(len(trigger)):
       ...
       while True:
         if self.summary[index] < self.summary[index + 1]:
           ...
           break
       maxes[i] = np.round(self.summary[index],3)
    ...
    for i in range(len(picks)):
      ...
      SNR[i] = np.round(maxes[i] / noise,1)
  ...
  def uncertainty(self):
    ...
    for i in range(len(trigger)):
      ...
      index0 = int(np.round((picks[i] - self.tr.stats.starttime) / dt, 0))
      index = int(np.round(trigger[i] / dt, 0))
      ...
      pick_uncert[i] = np.round(t[index+1] - (picks[i] - self.tr.stats.starttime), 3)
  ...
  def winlen(self,index,trigger_ptnl,filter_length,t,dt):
    ...
    if len(trigger_ptnl)==1:
      ...
      if trigger_ptnl[i]<=filter_length:
        r=int(np.round(trigger_ptnl[i]/dt,0))
      if trigger_ptnl[i]>filter_length:
        r=int(np.round(filter_length/dt,0))
      if trigger_ptnl[i]+filter_length>=t[-1]:
        R=int(np.round((t[-1]-trigger_ptnl[i])/dt,0))
      if trigger_ptnl[i]+filter_length<t[-1]:
        R=int(np.round(filter_length/dt,0))
      ...
      if i==0:
        if trigger_ptnl[i]<=filter_length:
          r=int(np.round(trigger_ptnl[i]/dt,0)); # print 'a'
        if  trigger_ptnl[i]>filter_length:
          r=int(np.round(filter_length/dt,0));  # print 'b'
        if (trigger_ptnl[i+1]-trigger_ptnl[i])<=filter_length:
          R=int(np.round((trigger_ptnl[i+1]-trigger_ptnl[i])/dt,0));  # print 'c'
        if (trigger_ptnl[i+1]-trigger_ptnl[i])>filter_length:
          R=int(np.round(filter_length/dt,0));  # print 'd'
      elif i>0 and i<len(trigger_ptnl)-1:
        if trigger_ptnl[i]-trigger_ptnl[i-1]<=filter_length:
          r=int(np.round((trigger_ptnl[i]-trigger_ptnl[i-1])/dt,0));  # print 'e'
        if trigger_ptnl[i]-trigger_ptnl[i-1]>filter_length:
          r=int(np.round(filter_length/dt,0));  # print 'f'
        if trigger_ptnl[i+1]-trigger_ptnl[i]<=filter_length:
          R=int(np.round((trigger_ptnl[i+1]-trigger_ptnl[i])/dt,0));  # print 'g'
        if trigger_ptnl[i+1]-trigger_ptnl[i]>filter_length:
          R=int(np.round(filter_length/dt,0));  # print 'h'
      elif i==len(trigger_ptnl)-1:
        if trigger_ptnl[i]-trigger_ptnl[i-1]<=filter_length:
          r=int(np.round((trigger_ptnl[i]-trigger_ptnl[i-1])/dt,0));  # print 'i'
        if trigger_ptnl[i]-trigger_ptnl[i-1]>filter_length:
          r=int(np.round(filter_length/dt,0));  # print 'j'
        if trigger_ptnl[i]+filter_length>t[-1]:
          R=int(np.round((t[-1]-trigger_ptnl[i])/dt,0));  # print 'k'
        if trigger_ptnl[i]+filter_length<=t[-1]:
          R=int(np.round(filter_length/dt,0));  # print 'l'
    return r,R
